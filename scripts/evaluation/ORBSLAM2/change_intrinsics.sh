...
sed -i "s/Camera.fx:.*/Camera.fx: $fx/g" $1/TUM1.yaml
sed -i "s/Camera.fy:.*/Camera.fy: $fy/g" $1/TUM1.yaml
sed -i "s/Camera.cx:.*/Camera.cx: $cx/g" $1/TUM1.yaml
sed -i "s/Camera.cy:.*/Camera.cy: $cy/g" $1/TUM1.yaml
...
if [ ${integer_fx} -gt 400 ]; then
  ...
  sed -i "s/Camera.width:.*/Camera.width: 848/g"   $1/TUM1.yaml
  sed -i "s/Camera.height:.*/Camera.height: 480/g" $1/TUM1.yaml
  sed -i "s/Camera.fps:.*/Camera.fps: 60/g"       $1/TUM1.yaml
else
  echo "Less than 400 so 640 360   90"
  sed -i "s/Camera.width:.*/Camera.width: 640/g"   $1/TUM1.yaml
  sed -i "s/Camera.height:.*/Camera.height: 360/g" $1/TUM1.yaml
  sed -i "s/Camera.fps:.*/Camera.fps: 90/g"       $1/TUM1.yaml
fi
