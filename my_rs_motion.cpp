...
{
    std::vector<dfloat3> positions, normals;
...
public:
    ...
    void render_camera(dfloat3 theta)
    {
        ...
        // glRotatef(theta.x * 180 / dPI, 0, 0, -1);
        // glRotatef(theta.y * 180 / dPI, 0, -1, 0);
        // glRotatef((theta.z - dPI / 2) * 180 / dPI, -1, 0, 0);
    
        glRotatef(theta.x * 180 / dPI, 1, 0, 0);
        glRotatef(theta.y * 180 / dPI, 0, 1, 0);
        glRotatef(theta.z * 180 / dPI, 0, 0, 1);
...
public:
    ...
    {   
        std::cout << "gyro data: " << gyro_data << std::endl;
        ...
        std::cout << "Added theta: " << theta.x << " " << theta.y << " " << theta.z << std::endl;
    }
...
int main(int argc, char * argv[]) try
{
    ...
    while (app)
    {
        ...
        std::cout << "Algo: " << algo.get_theta().x << " " << algo.get_theta().y << " " << algo.get_theta().z << std::endl;
    }
    // Stop the pipeline
    pipe.stop();
