...
struct drect
{
    ...
    drect adjust_ratio(dfloat2 size) const
    {
...
void set_viewport(const drect& r)
{
...
{
public:
    void render(const rs2::motion_frame& frame, const drect& r)
    {
...
private:
    ...
    void draw_motion(const rs2::motion_frame& f, const drect& r)
    {
        if (!_gl_handle)
    ...
    dfloat2 xyz_to_xy(float x, float y, float z, GLfloat model[], GLfloat proj[], float vec_norm)
    {
    ...
    static void draw_circle(float xx, float xy, float xz, float yx, float yy, float yz, float radius = 1.1, dfloat3 center = { 0.0, 0.0, 0.0 }, float intensity = 0.5f)
    {
        const auto N = 50;
...
{
public:
    void render(const rs2::pose_frame& frame, const drect& r)
    {
...
private:
    ...
    void draw_pose(const rs2::pose_frame& f, const drect& r)
    {
        if (!_gl_handle)
            glGenTextures(1, &_gl_handle);
...
{
    // Provide textual representation only
    void put_text(const std::string& msg, float norm_x_pos, float norm_y_pos, const drect& r)
    {
        set_viewport(r);
...
public:
    ...
    void show(const drect& r, float alpha = 1.f) const
    {
        if (!_gl_handle)
            return;
    ...
    void render(const rs2::frame& frame, const drect& rect, float alpha = 1.f)
    {
...
public:
    ...
    void show(const rs2::frame& frame, const drect& rect)
    {
    ...
    void show(const std::map<int, rs2::frame> frames)
    {
        // Render openGl mosaic of frames
         if (frames.size())
         {
             ...
             {
                 drect viewport_loc{ view_width * (stream_no % cols), view_height * (stream_no / cols), view_width, view_height };
...
private:
    ...
    void render_video_frame(const rs2::video_frame& f, const drect& r)
    {
        auto& t = _textures[f.get_profile().unique_id()];
        t.render(f, r);
    }

    void render_motion_frame(const rs2::motion_frame& f, const drect& r)
    {
        auto& i = _imus[f.get_profile().unique_id()];
        i.render(f, r);
    }

    void render_pose_frame(const rs2::pose_frame& f, const drect& r)
    {
        auto& i = _poses[f.get_profile().unique_id()];
        i.render(f, r);
    }

    void render_frameset(const rs2::frameset& frames, const drect& r)
    {
    ...
    drect calc_grid(drect r, size_t streams)
    {
        ...
        return drect{ static_cast<float>(w), static_cast<float>(h), static_cast<float>(new_w), static_cast<float>(new_h) };
    }

    std::vector<drect> calc_grid(drect r, std::vector<rs2::frame>& frames)
    {
        ...
        for (int i = 0; i < frames.size(); i++)
        {
            ...
            dfloat2 margin = { grid.w * 0.02f, grid.h * 0.02f };
            auto r = drect{ cell_x_postion + margin.x, cell_y_position + margin.y, grid.w - 2 * margin.x, grid.h };
            rv.push_back(r.adjust_ratio(dfloat2{ fw, fh }));
        }
